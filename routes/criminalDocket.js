...
const CriminalDocket = require('../models/CriminalDocket');
...
router.post('/', verifyToken, async (req, res) => {
  try {
    ...
    const docket = new CriminalDocket({
      ...req.body,
...
router.get('/my', verifyToken, async (req, res) => {
  try {
    const dockets = await CriminalDocket.find({ submittedBy: req.user.userId })
      .sort({ createdAt: -1 });
    res.json(dockets);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
router.get('/:id', verifyToken, async (req, res) => {
  try {
    const docket = await CriminalDocket.findById(req.params.id);
    res.json(docket);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
router.put('/:id', verifyToken, async (req, res) => {
  try {
    const updated = await CriminalDocket.findByIdAndUpdate(
      req.params.id,
      req.body,
      { new: true }
    );
...
router.get('/admin/all', verifyToken, verifyAdmin, async (req, res) => {
  ...
  try {
    const dockets = await CriminalDocket.find({
...
router.patch('/view/:id', verifyToken, verifyAdmin, async (req, res) => {
  try {
    await CriminalDocket.findByIdAndUpdate(
      req.params.id,
      { adminViewed: true },
      { new: true }
    );
...
// Add this route to your criminal-dockets routes
...
const storage = multer.diskStorage({
  destination: (req, file, cb) => {
    const uploadPath = path.join(__dirname, '../uploads/criminal-dockets'); // Points to backend/uploads/criminal-dockets
...
router.post('/upload', verifyToken, upload.single('file'), async (req, res) => {
  try {
    ...
    const docket = await CriminalDocket.findById(docketId);
    if (!docket) {
      fs.unlinkSync(req.file.path);
    ...
    const fileData = {
      ...
      size: req.file.size,
  url: `/uploads/criminal-dockets/${req.file.filename}` // Use relative URL
    };

    const updatedDocket = await CriminalDocket.findByIdAndUpdate(
      docketId,
...
router.delete('/delete-file', verifyToken, async (req, res) => {
  try {
    ...
    const filePath = path.join(__dirname, '../uploads/criminal-dockets', filename);
