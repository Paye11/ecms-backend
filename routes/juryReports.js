...
const JuryReport = require('../models/JuryReport');
...
router.get('/my', verifyToken, async (req, res) => {
  try {
    const reports = await JuryReport.find({ submittedBy: req.user.userId })
      .sort({ createdAt: -1 });
    res.json(reports);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
router.get('/:id', verifyToken, async (req, res) => {
  try {
    const report = await JuryReport.findById(req.params.id);
    res.json(report);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
router.put('/:id', verifyToken, async (req, res) => {
  try {
    const updated = await JuryReport.findByIdAndUpdate(
      req.params.id,
      req.body,
      { new: true }
    );
...
router.get('/admin/all', verifyToken, verifyAdmin, async (req, res) => {
  ...
  try {
    const reports = await JuryReport.find({
...
router.patch('/view/:id', verifyToken, verifyAdmin, async (req, res) => {
  try {
    await JuryReport.findByIdAndUpdate(
      req.params.id,
      { adminViewed: true },
      { new: true }
    );
...
const storage = multer.diskStorage({
  destination: (req, file, cb) => {
    const uploadPath = path.join(__dirname, '../uploads/jury-reports');
...
router.post('/upload', verifyToken, upload.single('file'), async (req, res) => {
  try {
    ...
    const docket = await JuryReport.findById(docketId);
    if (!docket) {
      fs.unlinkSync(req.file.path);
    ...
    const fileData = {
      ...
      size: req.file.size,
url: `/uploads/jury-reports/${req.file.filename}`
    };

    const updatedDocket = await JuryReport.findByIdAndUpdate(
      docketId,
...
router.delete('/delete-file', verifyToken, async (req, res) => {
  try {
    ...
    const filePath = path.join(__dirname, '../uploads/jury-reports', filename);
