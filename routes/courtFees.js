...
router.get('/my', verifyToken, async (req, res) => {
  try {
    const reports = await CourtFee.find({ submittedBy: req.user.userId })
      .sort({ createdAt: -1 });
    res.json(reports);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
router.get('/:id', verifyToken, async (req, res) => {
  try {
    const report = await CourtFee.findById(req.params.id);
    res.json(report);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
router.put('/:id', verifyToken, async (req, res) => {
  try {
    const updated = await CourtFee.findByIdAndUpdate(
      req.params.id,
      req.body,
      { new: true }
    );
...
router.get('/admin/all', verifyToken, verifyAdmin, async (req, res) => {
  ...
  try {
    const reports = await CourtFee.find({
...
router.patch('/admin/view/:id', verifyToken, verifyAdmin, async (req, res) => {
  try {
    await CourtFee.findByIdAndUpdate(
      req.params.id,
      { adminViewed: true },
      { new: true }
    );
...
// Configure multer storage for fee reports
const storage = multer.diskStorage({
  destination: (req, file, cb) => {
    const uploadPath = path.join(__dirname, '../uploads/fee-reports');
...
router.post('/upload', verifyToken, upload.single('file'), async (req, res) => {
  try {
    ...
    const docket = await CourtFee.findById(docketId);
    if (!docket) {
      fs.unlinkSync(req.file.path);
    ...
    const fileData = {
      ...
      size: req.file.size,
      url: `/uploads/fee-reports/${req.file.filename}`
    };

    const updatedDocket = await CourtFee.findByIdAndUpdate(
      docketId,
...
router.delete('/delete-file', verifyToken, async (req, res) => {
  try {
    ...
    const filePath = path.join(__dirname, '../uploads/fee-reports', filename);
